...
- The read-only WeKnora integration is an independently built service under `../services/weknora-mcp/`, connected through the existing `type: "http"` MCP path. DeerFlow receives only its `/mcp` URL and `X-MCP-Auth-Token`; the upstream WeKnora API key and knowledge-base allowlist stay in the service. Keep the example entry default-disabled, preserve the default `weknora_` tool prefix, and validate the boundary with `tests/test_weknora_mcp.py`. Deployment, live verification, and rollback are documented in [docs/WEKNORA_MCP.md](docs/WEKNORA_MCP.md).
