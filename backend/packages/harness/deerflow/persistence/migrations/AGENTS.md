...
`0024_project_documents` → both `0025_repair_run_change_seq` and the
historical `0025_run_change_seq_repair` →
`0026_merge_run_change_seq` → `0026_mcp_task_lease_tokens` (current head). The two 0025 repairs
contain equivalent guarded DDL but were independently shipped before a branch
merge; preserve both IDs and merge them rather than manually rewriting an
existing database's `alembic_version` row. Alembic's default
`alembic_version.version_num` is `VARCHAR(32)`, so every revision ID, including
the merge ID, must be 32 characters or shorter. The preference
...
- `migrations/versions/0025_repair_run_change_seq.py` and `0025_run_change_seq_repair.py` — two independently published IDs for the same idempotent repair of the schema skipped when `0023_run_change_seq` was inserted ahead of the already-shipped `0023_user_preferences` (#5516). Both re-apply guarded `run_change_clock`, `runs.change_seq`, and cursor-index DDL after `0024_project_documents`; neither owns or removes the ancestor schema on downgrade. `0026_merge_run_change_seq.py` joins the branch stamps without DDL, so either existing database upgrades normally. `tests/test_run_change_repair_history.py` reconstructs pre-insertion published descendants and verifies historical upgrade, unchanged healthy positions, and usable run-store writes after downgrade and re-upgrade.
- `migrations/versions/0026_mcp_task_lease_tokens.py` — chains after `0026_merge_run_change_seq` and adds nullable `mcp_tasks.lease_token` / `notification_lease_token` columns so every poll, cancel, and notification mutation can be fenced to the exact claim generation
