...
- `migrations/versions/0025_run_change_seq_repair.py` — idempotently repeats the `0023_run_change_seq` schema work at a new head. `0023_run_change_seq` was inserted behind the already-shipped `0023_user_preferences` / `0024_project_documents` revisions, so databases previously stamped at 0024 skipped its new column, clock table, and indexes even though Alembic reported them current. The repair preserves existing rows and clock values; its downgrade is intentionally a no-op because the canonical 0023 revision owns those objects and removing them at 0024 would leave the runtime schema invalid.
