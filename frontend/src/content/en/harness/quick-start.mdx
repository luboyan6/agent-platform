---
title: Quick Start
description: Learn how to create and run a XIAOE agent with create_deerflow_agent, from model setup to streaming responses.
---
...
<Callout type="info" emoji="🚀">
  This guide shows you how to build and run a XIAOE agent in Python with
  <code>create_deerflow_agent</code>.
</Callout>

The fastest way to understand XIAOE Harness is to create an agent directly in code. This quick start walks through model setup, agent creation, and streaming a response.

## Prerequisites

XIAOE Harness requires Python 3.12 or later. The package is part of the `deerflow` repository under `backend/packages/harness`.
...
This returns a compiled LangGraph agent with XIAOE's default middleware chain.
...
## When to use XIAOEClient instead
