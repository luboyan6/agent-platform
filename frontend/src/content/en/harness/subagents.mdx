...
XIAOE ships with two built-in subagents:
...
In addition to the built-in subagents, XIAOE supports delegating to external agents through the **Agent Client Protocol (ACP)**. ACP allows XIAOE to invoke agents running as separate processes (including third-party CLI tools wrapped with an ACP adapter).
...
<Callout type="tip">
  ACP agents run as child processes managed by XIAOE. They communicate over
