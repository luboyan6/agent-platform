...
test("aboutMarkdown heading reflects the package version when env is unset", async () => {
  ...
  // empty or undefined APP_VERSION interpolation (`About XIAOE ]` /
  // `About XIAOE undefined]`), not just removal of the old literal.
  expect(aboutMarkdown).toContain(`[About XIAOE ${APP_VERSION}]`);
});
